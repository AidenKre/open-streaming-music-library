...
class Database:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            print(f"Error loading sqlite init script, found at path {self.context.init_sql_path} with exception {e}")
            conn.rollback()
            conn.close()
    ...
    def add_track(self, track: Track, timeout: float = 5) -> bool:
        if track.metadata.is_empty():
            print(f"empty metadata track passed to Database.add_track(): {track.metadata}")
            return False
    ...
    def delete_track(self, uuid_id: str, timeout: float = 5) -> bool:
        ...
        try:
            trackmetadata_cursor.execute("DELETE FROM trackmetadata WHERE uuid_id = ?", (uuid_id,))
        except Exception as e:
    ...
    def get_tracks(self, search_parameters: dict, timeout: float = 5) -> List[Track]:
        ...
        try:
            ...
            print(f"Failed to search database. search_parameters: {search_parameters}. Exception: {e}")
            conn.close()
            return []
        finally:
            conn.close()
