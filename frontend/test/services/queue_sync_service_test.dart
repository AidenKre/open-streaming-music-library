...
void main() {
  ...
  test('scheduleSync POSTs queue state after the debounce', () async {
    Uri? capturedUrl;
    ...
    service.scheduleSync(sessionId: 7, currentPlayPosition: 0, quality: '256');

    await Future<void>.delayed(const Duration(milliseconds: 700));

    expect(capturedUrl?.path, '/queue/sync');
  ...
  test('respects currentPlayPosition when fetching entries', () async {
    ...
    service.scheduleSync(sessionId: 1, currentPlayPosition: 1, quality: '192');
    await Future<void>.delayed(const Duration(milliseconds: 700));
  ...
  test('debounces rapid calls into a single POST', () async {
    ...
    service.scheduleSync(sessionId: 1, currentPlayPosition: 0, quality: '320');
    service.scheduleSync(sessionId: 1, currentPlayPosition: 0, quality: '320');
    service.scheduleSync(sessionId: 1, currentPlayPosition: 0, quality: '320');
  ...
  test('does nothing when sessionId is null', () async {
    ...
    service.scheduleSync(sessionId: null, currentPlayPosition: 0, quality: '320');
    await Future<void>.delayed(const Duration(milliseconds: 700));
