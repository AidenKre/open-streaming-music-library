...
void main() {
  ...
  test('deleteDownloadsForUuids handles cleanup in multiple chunks', () async {
    final uuids = List.generate(450, (i) => 'bulk-$i');
      ...
      for (final uuid in uuids) {
        batch.insert(
          ...
          TracksCompanion.insert(
            ...
            createdAt: 0,
            lastUpdated: 0,
            filePath: Value(p.join(tempDir.path, '$uuid.mp3')),
    ...
    final initialVersion = manager.downloadStatusVersion.value;

    await manager.deleteDownloadsForUuids(uuids);

    final rows = await db.select(db.tracks).get();
    expect(rows, hasLength(uuids.length));
    expect(rows.every((row) => row.filePath == null), isTrue);
    expect(rows.every((row) => row.downloadedBitrateKbps == null), isTrue);
    expect(rows.every((row) => row.fileSizeBytes == null), isTrue);
    expect(rows.every((row) => row.downloadedQuality == null), isTrue);
    expect(manager.downloadStatusVersion.value, initialVersion + 1);
  });
  ...
  test('reset waits for a post-stream worker before deleting files', () async {
    await _insertTrack(db, 'abc');
    final manager = buildManager(
      client: MockClient((_) async => http.Response.bytes([1, 2, 3], 200)),
    );
    ...
    final releaseHook = Completer<void>();
    addTearDown(() {
      if (!releaseHook.isCompleted) releaseHook.complete();
      manager.dispose();
    });
    manager.testHookBeforeDbWrite = (_) async {
      if (!hookEntered.isCompleted) hookEntered.complete();
    ...
    final resetFuture = manager.resetAndDeleteFiles();
    unawaited(resetFuture.whenComplete(() => resetCompleted = true));
  ...
  test(
    'enqueueTracksAtQuality replaces finished history for the same uuid',
    () async {
      await _insertTrack(db, 'abc');
      ...
      final manager = buildManager(
        client: MockClient((req) async {
          ...
          return http.Response.bytes(
            [requestedQualities.length],
            200,
            headers: {
              'x-audio-bitrate-kbps': requestedQualities.length == 1
                  ? '128'
                  : '320',
              'x-audio-extension': 'mp3',
            },
      ...
      await manager.enqueueTracksAtQuality([_track('abc')], quality: '128');
      await _waitForFinish(manager);
      ...
      await manager.enqueueTracksAtQuality([
        _track(
          ...
          downloadedBitrateKbps: firstRow.downloadedBitrateKbps,
          downloadedQuality: firstRow.downloadedQuality,
        ),
      ], quality: '320');

      await _waitFor(() => requestedQualities.length == 2);
      await _waitFor(() => manager.snapshot().every((j) => !j.isActive));

      expect(requestedQualities, ['128', '320']);
      final jobs = manager
          .snapshot()
          .where((job) => job.uuidId == 'abc')
          .toList(growable: false);
      expect(jobs, hasLength(1));
      expect(jobs.single.isCompleted, isTrue);

      final secondRow = await (db.select(
        db.tracks,
      )..where((t) => t.uuidId.equals('abc'))).getSingle();
      expect(secondRow.downloadedQuality, '320');
    },
  );
  ...
  test('dispose cancels an active worker before it can commit', () async {
    await _insertTrack(db, 'abc');
    final stream = StreamController<List<int>>.broadcast();
    final manager = buildManager(client: _StreamingClient(stream.stream));

    await manager.enqueueTracks([_track('abc')], quality: '320');
    await _waitFor(() => manager.snapshot().any((job) => job.isActive));
